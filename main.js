  ...
  updateForces() {
    ...
    if (firing && shotCount <= shotCountMax && coolDown == ready) {
      if (this.count >= 3) {
        ...
        bullets.push(new Bullet(this.angle, 15, this.x + Math.cos(this.angle) * (Math.floor(Math.random() * (101)) + 100), this.y + Math.sin(this.angle) * (Math.floor(Math.random() * (101)) + 100), this.vx + Math.cos(this.angle) * 600  + Math.floor(Math.random() * 4) - 2, this.vy + Math.sin(this.angle) * 600  + Math.floor(Math.random() * 4) - 2, "red", false, 0, false));
        shotCount += 1;
        this.count = 0;
      }
      this.count += 1
...
//Bullets class
class Bullet {
  constructor(direction, size, x, y, vx, vy, color, flak, time, enemy) {
    //declaring vars
  ...
  update(i) {
    ...
    //check if bullet in correct height else dellete
    if (this.y > 10) {
      ctx.save();
      ctx.translate(window.innerWidth / 2, window.innerHeight / 2);
      ctx.scale(zoomLevel,zoomLevel);
    ...
    } else if (this.size != null) {this.delete(i)}
    ...
    //if bullet is player bullet then check all enemeis to see if there is a hit
    if (this.enemy == false) {
    for (let i = 0; i < enemyPlanes.length; i++) {
      let dist = Math.sqrt(Math.pow(-this.y + enemyPlanes[i].y, 2) + Math.pow(-this.x + enemyPlanes[i].x, 2))
      if (dist < r1) {r1 = dist; f = i;};}};

    //enemy bullets
    if (r < 500 && this.enemy == true) {f22.health -= 1; this.delete(i);};
  
    //plane bullets
    if (r1 < 700 && this.enemy == false) {this.delete(i); enemyPlanes[f].health -= 1; console.log('hit')};

    //if its flak
    if (this.enemy == false) {
      let a = Math.atan2(enemyPlanes[f].y - this.y, enemyPlanes[f].x - this.x)
      this.vx += Math.cos(a) * 1
      this.vy += Math.sin(a) * 1
    }

    if (this.maxTime <= this.time && this.flak) {this.delete(i)};}
    
    

  delete(i) {
    //if bullet needs to be deleted here you can delete it
    bullets.shift(i, 1);
    this.direction = null;
    this.size = null;
    this.x = null;
...
class enemyTurret {
  ...
  update() {
    if (Math.sqrt((this.x - f22.x) ** 2 + (this.y + f22.y) ** 2) <= 8000 / zoomLevel) {
      ...
      this.c += 1;
      if (this.c > 15) {
        bullets.push(new Bullet(this.turretAngle, 30, this.x + 500 + (Math.cos(this.turretAngle) * 2500) ,  this.y + 500 + (Math.sin(this.turretAngle) * 2500), Math.cos(this.turretAngle) * 200, Math.sin(this.turretAngle) * 200, "red", true, 200, true));
        this.c = 0;
    }}}
...
class enemyPlane {
  constructor (x, y ,vx, vy, angle, v, index) {
    //General Information
    this.x = x;
    this.y = y;
    this.vx = vx;
  ...
  updateForces() {
    ...
    if ((this.y < 10 && this.vy < 0 )|| this.health <= 0) {this.delete()};
    ...
    if (r < 9000) {
      if (Math.abs(this.angle - a) < Math.PI / 4 && this.shots <= this.maxShots && this.coolTime == this.coolDown) {
        if (this.time >= 5) {
          bullets.push(new Bullet(this.angle, 15, this.x + Math.cos(this.angle) * (Math.floor(Math.random() * (101)) + 100), this.y + Math.sin(this.angle) * (Math.floor(Math.random() * (101)) + 100), this.vx + Math.cos(this.angle) * 600  + Math.floor(Math.random() * 4) - 2, this.vy + Math.sin(this.angle) * 600  + Math.floor(Math.random() * 4) - 2, "red", false, 0, true));
          this.shots += 1;
          this.time = 0;
          }
  ...
  calcAngle() {
    ...
    if (this.time % 2 == 0) {
      this.a = ((Math.atan2(-this.y + f22.y + f22.vy, -this.x + f22.x + f22.vx) + Math.PI * 2) % (Math.PI * 2))
    }
    if (this.y < 10000) {this.a = Math.PI / 2}


    this.angle = (this.angle + Math.PI * 2) % (Math.PI * 2)
    let dif = (this.a - this.angle);
    let f = 0;
  ...
  delete() {
    ...
    this.coolDown = null;
    this.coolTime = null;
    enemyPlanes.splice(this.index,1);
    this.index = null;
  }
}
...
for (let i = 0; enemyCount > i; i++) {
  enemyPlanes.push(new enemyPlane(0,i * 1000 + 10000, 0, 0, 0, 0, i))
}
...
loadResources(imageUrls)
  .then(function(resources) {
    ...
    f22 = new Player("f22", 0, 30000, 0, 0, 0, 0, 20, 0.003125, Math.PI / 60, 0, 1, true, 100);
